...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		physics::LinkPtr gripper  = model->GetLink(GRIP_BASE);
		physics::LinkPtr link2   = model->GetLink(LINK2_NAME);
		physics::LinkPtr gripper_middle = model->GetLink(GRIP_NAME);
		...
		const math::Box& middleBBox = gripper_middle->GetBoundingBox();
		...
		*/ 
		...
		else
		{
			...
			if( episodeFrames > 1)
			{
				...
				rewardHistory = -0.125;//avgGoalDelta*0.3; //avgGoalDelta*960 + 100*(distLink2-distGoal);
